...
export default function ModuloDetalleScreen() {
  ...
  if (modulo.tipoConexion === 'api') {
        ...
        // LOG: Mostrar lo que se enviará al backend
        console.log('🚀 Enviando al backend (API):');
        console.log('URL:', modulo.apiRestUrl);
        console.log('Body:', JSON.stringify(logRequestBody, null, 2));
        ...
        // LOG: Mostrar lo que se enviará al backend
        console.log('🚀 Enviando al backend (DB directa):');
        console.log('URL:', modulo.apiRestUrl);
        console.log('Body:', JSON.stringify(requestBodyLog, null, 2));
      ...
      // LOG: Mostrar el body crudo de la respuesta
      const rawResponse = await response.text();
      console.log('⬅️ Respuesta RAW del backend:');
      console.log(rawResponse);
      let resultado;
      try {
        resultado = JSON.parse(rawResponse);
      } catch (e) {
        console.error('❌ Error al parsear JSON de la respuesta:', e);
        throw new Error('Respuesta del backend no es JSON válido');
      }
      console.log('📦 Respuesta recibida del servidor (parseada)');
  ...
  console.log('🎯 ========================================');
  console.log('🎯 PUNTO DE RENDERIZADO PRINCIPAL');
  console.log('🎯 ========================================');
  console.log('🎯 ID del módulo:', id);
  console.log('🎯 Módulo existe:', !!modulo);
  console.log('🎯 Nombre del módulo:', modulo?.nombre);
  ...
  return (
    <SafeAreaView style={styles.container}>
          ...
          <View style={styles.errorContainer}>
            ...
            {error.includes('no tiene configurada') ? (
              <TouchableOpacity
                ...
                onPress={() => {
                  console.log('🔧 ========================================');
                  console.log('🔧 CLICK EN BOTON DE CONFIGURACION (ERROR VIEW)');
                  console.log('🔧 ========================================');
                  console.log('🔧 Navegando a configurarModulo con id:', id);
                  router.push({
                    pathname: '/modulos/configurarModulo',
                    params: { id: id as string }
                  });
                }}
              >
                <Ionicons name="settings-outline" size={20} color="#1976d2" />
                <Text style={{ color: '#1976d2', marginLeft: 8 }}>Configurar Módulo</Text>
              </TouchableOpacity>
            ) : (
      ...
      {/* Botón de configuraciones fijo abajo a la derecha - SIEMPRE visible */}
      <TouchableOpacity
        style={styles.configButton}
        onPress={() => {
          console.log('🔧 ========================================');
          console.log('🔧 CLICK EN BOTON DE CONFIGURACION');
          console.log('🔧 ========================================');
          console.log('🔧 Navegando a configurarModulo con id:', id);
          console.log('🔧 Módulo actual:', modulo?.nombre);
          console.log('🔧 Tiene submódulos?:', modulo?.tieneSubmodulos);
          console.log('🔧 Cantidad real de submódulos:', modulo?.submodulos?.length || 0);
          // Alternativa para web - usa objeto con pathname y params
          router.push({
            pathname: '/modulos/configurarModulo',
            params: { id: id as string }
          } as any);
