...
const prettyDateTime = (v?: any) => {
    if (!v) return '—';
    const d = new Date(String(v));
    return isNaN(d.getTime()) ? String(v) : d.toLocaleString();
};

export default function ControlDashboardBarcoder() {
    ...
    const ENDPOINT = `${API_URL}/control-optima/barcoder`;
    ...
    const fetchPage = useCallback(async (pageToLoad: number, replace = false) => {
        ...
        try {
            ...
            console.error('[Barcoder] error:', e);
            if (replace) { setRows([]); setTotal(0); setAgg({ piezas: 0, area: 0 }); }
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    const headerMetrics = useMemo(() => (
        <View style={styles.headerRow}>
            <View style={styles.kpi}>
                ...
                <Text style={styles.kpiText}>Items: {rows.length} / {total}</Text>
            </View>
            <View style={styles.kpi}>
                <Ionicons name="cube-outline" size={18} />
                <Text style={styles.kpiText}>Piezas: {agg.piezas}</Text>
            </View>
            <View style={styles.kpi}>
                <Ionicons name="layers-outline" size={18} />
                <Text style={styles.kpiText}>Área: {agg.area.toFixed(2)}</Text>
            </View>
        </View>
