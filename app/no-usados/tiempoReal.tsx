...
export default function ControlTerminalesScreen() {
  ...
  const isTablet = !isWeb && windowWidth >= 600 && windowWidth < 1024;
  const isMobile = !isWeb && windowWidth < 600;
  let numColumns = 1;
  if (isWeb) numColumns = 4;
  else if (isTablet) numColumns = 2;
  else numColumns = 1;
  // Card width for web/tablet/mobile
  const gap = 8;
  const computedCardWidth = isWeb
    ? Math.max(150, Math.floor((windowWidth - 40 - gap * (numColumns - 1)) / numColumns))
    : isTablet
      ? Math.floor((windowWidth - 32 - gap * (numColumns - 1)) / numColumns)
      : Math.floor((windowWidth - 32));
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Date pickers: calendario visual en web, DateTimePicker en móvil */}
      ...
      >
        <View style={[styles.modalContainer, isMobile ? styles.modalContainerMobile : { padding: 12 }, { width: '100%' }]}>
          ...
          {modalGroupBy === 'none' ? (
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={[styles.card, { width: computedCardWidth, marginHorizontal: 4, marginVertical: 4, padding: 8 }]}>
                  {modalContext !== 'operador' && (
            ...
            (() => {
              ...
              return (
                <FlatList
                  ...
                  style={{ flex: 1, width: '100%' }}
                  renderItem={({ item: g }) => (
                    <View style={{ marginBottom: 8, width: computedCardWidth, marginHorizontal: 4 }}>
  ...
  <View style={{ flex: 1, paddingHorizontal: 0, paddingVertical: 8, width: '100%', alignSelf: 'stretch' }}>
        ...
        {loadingTiempo ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity key={item.key} style={[styles.card, isHighlighted ? { backgroundColor: '#fff7e6' } : undefined, { width: computedCardWidth, marginHorizontal: 4, marginVertical: 4 }]} onPress={() => {
