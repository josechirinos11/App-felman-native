...
export default function ControlProduccionScreen() {
  ...
  const [desde, setDesde] = useState<string>(formatDateOnly(new Date().toISOString()));
  ...
  const fetchByRange = async () => {
    ...
    setLoadingData(true);
    try {
      // 1) Intentar GET con query params
      const qs = `?desde=${encodeURIComponent(desde)}&hasta=${encodeURIComponent(hasta)}`;
      let res = await fetch(`${API_URL}/control-terminales/tiempo-real-nueva${qs}`);
      if (!res.ok) {
        // 2) Fallback POST con JSON
        res = await fetch(`${API_URL}/control-terminales/tiempo-real-nueva`, {
          method: 'POST',
      ...
      console.error('fetchByRange error', e);
      setRows([]);
    } finally {
