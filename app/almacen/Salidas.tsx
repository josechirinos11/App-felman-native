...
export default function Salidas() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    cantidad: '', motivo: '', referencia: '', actualizado_por: ''
  });
  ...
  useEffect(() => {
    fetch(`${API_BASE}/control-almacen/salidas`)
      .then(res => res.json())
      .then(json => json.status === 'ok' && setData(json.data))
      .catch(err => console.error('Fetch salidas error:', err))
  ...
  const handleSubmit = async () => {
    ...
    const url = isUpdate
      ? `${API_BASE}/control-almacen/salidasactualizar/${currentItem.id}`
      : `${API_BASE}/control-almacen/salidascrear`;
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    return (
      ...
      >
        <Text style={styles.cardTitle}>Salida #{item.id}</Text>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <View style={styles.itemCount}>
            ...
            <Text style={styles.statusText}>{filtered.length} salidas</Text>
          </View>
        </View>
        ...
        { !isWebLarge && (
          <View style={styles.toolbarButtonsMobile}>
            <TouchableOpacity style={styles.addBtn} onPress={() => openModal({})}>
              <Text style={styles.addBtnText}>Agregar Salida</Text>
            </TouchableOpacity>
            <TouchableOpacity
      ...
      <Modal visible={modalVisible} animationType="slide">
        <View style={styles.modalContainer}>
          <Text style={styles.modalHeader}>{currentItem?.id ? 'Actualizar Salida' : 'Agregar Salida'}</Text>
          <ScrollView>
