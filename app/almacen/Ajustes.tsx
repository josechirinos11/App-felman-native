...
export default function Ajustes() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    tipo_ajuste: '', cantidad: '', motivo: '', actualizado_por: ''
  });
  ...
  useEffect(() => {
    fetch(`${API_BASE}/control-almacen/ajustes`)
      .then(res => res.json())
      .then(json => json.status === 'ok' && setData(json.data))
      .catch(err => console.error('Fetch ajustes error:', err))
      .finally(() => setLoading(false));
  }, []);

  const filtered = data.filter(item =>
    String(item.articulo_id).toLowerCase().includes(query.toLowerCase()) ||
    (item.tipo_ajuste || '').toLowerCase().includes(query.toLowerCase()) ||
    (item.motivo || '').toLowerCase().includes(query.toLowerCase())
  );
  ...
  const handleSubmit = async () => {
    ...
    const url = isUpdate
      ? `${API_BASE}/control-almacen/ajustesactualizar/${currentItem.id}`
      : `${API_BASE}/control-almacen/ajustescrear`;
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    return (
      ...
      >
        <Text style={styles.cardTitle}>Ajuste #{item.id}</Text>
        <View style={styles.cardContent}>
          <Text style={styles.cardText}>Artículo ID: {item.articulo_id}</Text>
          <Text style={styles.cardText}>Tipo: {item.tipo_ajuste}</Text>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <View style={styles.itemCount}>
            ...
            <Text style={styles.statusText}>{filtered.length} ajustes</Text>
          </View>
        </View>
        ...
        { !isWebLarge && (
          <View style={styles.toolbarButtonsMobile}>
            <TouchableOpacity style={styles.addBtn} onPress={() => openModal({})}>
              <Text style={styles.addBtnText}>Agregar Ajuste</Text>
            </TouchableOpacity>
            <TouchableOpacity
      ...
      <Modal visible={modalVisible} animationType="slide">
        <View style={styles.modalContainer}>
          <Text style={styles.modalHeader}>{currentItem?.id ? 'Actualizar Ajuste' : 'Agregar Ajuste'}</Text>
          <ScrollView>
            <View style={styles.inputGroup}>
              <Text style={styles.inputLabel}>Tipo de Ajuste</Text>
              <TextInput
                style={styles.input}
                value={form.tipo_ajuste}
                onChangeText={text => setForm({ ...form, tipo_ajuste: text })}
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                value={form.motivo}
                onChangeText={text => setForm({ ...form, motivo: text })}
