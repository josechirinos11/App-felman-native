...
export default function Configuracion() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/control-almacen/configuraciones`)
      .then(res => res.json())
      .then(json => json.status === 'ok' && setData(json.data))
      .catch(err => console.error('Fetch configuraciones error:', err))
      .finally(() => setLoading(false));
  }, []);

  const filtered = data.filter(item =>
    (item.clave || '').toLowerCase().includes(query.toLowerCase()) ||
    (item.valor || '').toLowerCase().includes(query.toLowerCase()) ||
  ...
  const handleSubmit = async () => {
    ...
    const url = isUpdate
      ? `${API_BASE}/control-almacen/configuracionesactualizar/${currentItem.id}`
      : `${API_BASE}/control-almacen/configuracionescrear`;
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    return (
      ...
      >
        <Text style={styles.cardTitle}>{item.clave}</Text>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <View style={styles.itemCount}>
            ...
            <Text style={styles.statusText}>{filtered.length} configuraciones</Text>
          </View>
        </View>
        ...
        { !isWebLarge && (
          <View style={styles.toolbarButtonsMobile}>
            <TouchableOpacity style={styles.addBtn} onPress={() => openModal({})}>
              <Text style={styles.addBtnText}>Agregar Configuración</Text>
            </TouchableOpacity>
            <TouchableOpacity
      ...
      <Modal visible={modalVisible} animationType="slide">
        <View style={styles.modalContainer}>
          <Text style={styles.modalHeader}>{currentItem?.id ? 'Actualizar Configuración' : 'Agregar Configuración'}</Text>
          <ScrollView>
            <View style={styles.inputGroup}>
              ...
              <TextInput
                style={styles.input}
                value={form.clave}
                onChangeText={text => setForm({ ...form, clave: text })}
