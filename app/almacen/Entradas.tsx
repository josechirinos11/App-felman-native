...
export default function Entradas() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    cantidad: '', referencia: '', actualizado_por: ''
  });
  ...
  useEffect(() => {
    fetch(`${API_BASE}/control-almacen/entradas`)
      .then(res => res.json())
      .then(json => json.status === 'ok' && setData(json.data))
      .catch(err => console.error('Fetch entradas error:', err))
  ...
  const handleSubmit = async () => {
    ...
    const url = isUpdate
      ? `${API_BASE}/control-almacen/entradasactualizar/${currentItem.id}`
      : `${API_BASE}/control-almacen/entradascrear`;
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    return (
      ...
      >
        <Text style={styles.cardTitle}>Entrada #{item.id}</Text>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <View style={styles.itemCount}>
            ...
            <Text style={styles.statusText}>{filtered.length} entradas</Text>
          </View>
        </View>
        ...
        { !isWebLarge && (
          <View style={styles.toolbarButtonsMobile}>
            <TouchableOpacity style={styles.addBtn} onPress={() => openModal({})}>
              <Text style={styles.addBtnText}>Agregar Entrada</Text>
            </TouchableOpacity>
            <TouchableOpacity
      ...
      <Modal visible={modalVisible} animationType="slide">
        <View style={styles.modalContainer}>
          <Text style={styles.modalHeader}>{currentItem?.id ? 'Actualizar Entrada' : 'Agregar Entrada'}</Text>
          <ScrollView>
